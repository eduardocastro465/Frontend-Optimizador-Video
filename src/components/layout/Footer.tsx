...
export default function Footer() {
  return (
    <footer className="relative overflow-hidden bg-[#07070f]">
      {/* Gradientes cinematográficos — igual que HeroPanel */}
      ...
      <div className="relative z-10 px-6 lg:px-20">
        {/* ── CTA strip ── */}
        <div className="relative flex flex-col sm:flex-row items-center justify-between gap-6 py-8 border-b border-white/[0.06]">
          <div>
          ...
          <div className="w-fit">
            <PrimaryButton className="px-8 py-3">Registrate</PrimaryButton>
          </div>
        </div>

        {/* ── Columnas ── */}
        <div className="grid grid-cols-1 md:grid-cols-4 gap-10 py-2">
          {/* Brand */}
          <div className="md:col-span-1">
            <div className="flex items-center gap-2.5 mb-5">
              <div
                className="w-8 h-8 rounded-xl bg-gradient-to-br from-blue-500/30 to-violet-500/30
                              border border-violet-500/30 flex items-center justify-center text-base"
              >
                🎥
              </div>
            ...
            <p className="font-body text-caption text-white/35 dark:text-white/30 leading-relaxed mb-6">
              ...
              <ul className="space-y-3">
                {items.map((item) => (
                  <li key={item}>
                    <a
                      href="#"
                      className="font-body text-caption text-white/35 hover:text-white/65
                                 transition-colors duration-150 flex items-center gap-1.5 group"
                    >
                      <span
                        className="w-0 group-hover:w-2 h-px bg-violet-400/60
